...
export const ComponentTooltip: React.FC<ComponentTooltipProps> = ({
  ...
  // Calculate tooltip dimensions - narrower and with more space between label and value
  const padding = 1.8;
  const lineHeight = 2.8;
  const tooltipWidth = 20;
  const tooltipHeight = padding * 2 + (tooltipEntries.length + 1) * lineHeight; // +1 for title
  const verticalOffset = 6; // Increased vertical offset to position tooltip higher
        ...
        return (
          <React.Fragment key={`entry-${i}`}>
            ...
            <Text
              ...
              text={entry.value}
              fontSize={2}
              fontStyle="bold" // Values are bold now
              fontFamily="Arial"
              fill="#333333"
              width={(tooltipWidth - padding * 2) / 1.7} // More space for values
              align="right"
            />
          </React.Fragment>
        );
